...
void VibroloAudioProcessor::parameterChanged  (const juce::String &parameterID, float newValue)
{
    ...
    if (parameterID.compare("vibeRate")==0)
    {
        vibeRate = newValue;
    }
...
{
    m_processor->setChannels(2);
    m_processor->setSampleRate(getSampleRate());
    m_processor->clear();
    m_buffer.resize(2*samplesPerBlock);
...
void VibroloAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    auto vlfoIncrement = drow::MathConstants<float>::twoPi/getSampleRate();
    auto tlfoIncrement = drow::MathConstants<float>::twoPi/getSampleRate();
    
    m_processor->setPitchSemiTones(vibe*sinf(vibeLfo));
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        {
            vibeLfo += vlfoIncrement*vibeRate;
            if(tremRate<0.15f)
                tremRate = 0.15f;
            tremLfo += tlfoIncrement*tremRate*10;
            
            m_buffer[sample*totalNumInputChannels+channel] = buffer.getSample(channel,sample)*((1-trem)+(trem)*sinf(tremLfo));
        }
        
    }
    
    m_processor->putSamples(m_buffer.data(), buffer.getNumSamples());
    
    if(m_processor->numSamples()>=buffer.getNumSamples())
    {
        m_processor->receiveSamples(m_buffer.data(), buffer.getNumSamples());
        ...
        for (int channel = 0; channel < totalNumInputChannels; ++channel)
        {
            ...
            for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
            {
                buffer.setSample(channel,sample,m_buffer[sample*totalNumInputChannels+channel]);
            }
